...
const EmployerLoginForm = ({ onSwitchTab }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        login(data.token, data.employerId, data.employer);
        setTimeout(() => {
          router.push(
  ...
  const handleForgotSendOtp = async (e) => {
    e.preventDefault();
    if (!email) {
      showSnackbar("Please enter your registered email.", "error");
      return;
    }
    setForgotLoading(true);
    try {
      const res = await fetch("https://toc-bac-1.onrender.com/api/auth/employer/forgot-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      showSnackbar("If the email exists, an OTP has been sent.", "success");
    } catch (err) {
  ...
  const handleForgotReset = async (e) => {
    ...
    setForgotLoading(true);
    try {
      const res = await fetch("https://toc-bac-1.onrender.com/api/auth/employer/reset-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        showSnackbar(data?.message || "Failed to reset password.", "error");
      } else {
  ...
  return (
    <div className="animate-fade-in">
      <div className="text-center mb-6">
        <h1 className="text-3xl font-bold text-gray-900">{showForgot ? "Forgot Password" : "Employer Login"}</h1>
        <p className="text-gray-600 mt-2">{showForgot ? "Reset your password using the OTP sent to your email." : "Access your company dashboard."}</p>
      </div>
      {snackbar.open && (
        <div
      ...
      {!showForgot ? (
        <>
          <form onSubmit={handleSubmit} autoComplete="off" className="space-y-4">
            ...
            <div className="text-right">
              <button type="button" onClick={() => setShowForgot(true)} className="text-sm text-indigo-600 hover:underline">Forgot password?</button>
            </div>
            <button
              type="submit"
              disabled={loading}
        ...
        <form onSubmit={forgotOtpSent ? handleForgotReset : handleForgotSendOtp} autoComplete="off" className="space-y-4">
          <div>
          ...
          <div className="flex items-center justify-between">
            <button type="button" onClick={() => { setShowForgot(false); setForgotOtpSent(false); }} className="text-sm text-gray-600 hover:underline">Back to sign in</button>
            <button type="submit" disabled={forgotLoading} className="px-4 py-2 bg-indigo-600 text-white rounded-lg disabled:opacity-60">
              {forgotOtpSent ? (forgotLoading ? 'Resetting...' : 'Reset Password') : (forgotLoading ? 'Sending...' : 'Send OTP')}
            </button>
          </div>
        </form>
      )}
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        login(data.token, data.employer.id, data.employer);
